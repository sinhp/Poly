...
def ofComponents₂ {F G : 𝒞 ⥤ 𝒟 ⥤ ℰ}
    ...
    (naturality₂_left : ∀ {Γ Δ : 𝒞} (X : 𝒟) (σ : Γ ⟶ Δ),
      (F.map σ).app X ≫ (app Δ X).hom = (app Γ X).hom ≫ (G.map σ).app X := by aesop_cat)
    (naturality₂_right : ∀ {X Y : 𝒟} (Γ : 𝒞) (f : X ⟶ Y),
      (F.obj Γ).map f ≫ (app Γ Y).hom = (app Γ X).hom ≫ (G.obj Γ).map f := by aesop_cat) :
    F ≅ G :=
  NatIso.ofComponents
    (fun Γ => NatIso.ofComponents (app Γ) (fun f => by simpa using naturality₂_right Γ f))
    (fun σ => by ext X : 2; simpa using naturality₂_left X σ)
